					 ...
					 size_t count)
{
	...
	dev_info(dev, "Setting sleep mode to %ld.", new_value);
...
#define VKEYBOARD_ENABLE 12
#define VKEYBOARD_DISABLE 4

void rmi_VirtualKeyboardmode(int flag)
{
    ...
    if(flag == true)
    	reg1 = VKEYBOARD_ENABLE;
   	else
		reg1 = VKEYBOARD_DISABLE;

	retval = rmi_write(fc->rmi_dev, 0x0079, reg1);

	if (retval < 0)
		printk("[rmi]Failed to write virtual keyboard\n");

	retval = rmi_read(fc->rmi_dev, 0x0079, &reg1);

	//printk("rmi_VirtualKeyboardmode flag: %d, reg: %d\n", flag, reg1);
...
static int rmi_f01_resume(struct rmi_function_container *fc)
{
    ...
    if(keyboard_mode)
    {
        //printk("[Touch]Keyboard_mode_enable\n");    
        rmi_VirtualKeyboardmode(KEYBOARD_ON);
    }
    else
    {
       	//printk("[Touch]Keyboard_mode_disable\n");
        rmi_VirtualKeyboardmode(KEYBOARD_OFF);
    }
