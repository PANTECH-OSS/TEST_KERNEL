...
struct msm_hsic_hcd {
	...
	int			resume_status;
	int			resume_again;
// P13120, [PATCH] EHCI: HSIC: Implement new reset sequence to workaround PHY lockup issue [CRs-Fixed: 445659] --->>>
	int			bus_reset;
	int			reset_again;
// P13120, [PATCH] EHCI: HSIC: Implement new reset sequence to workaround PHY lockup issue [CRs-Fixed: 445659] ---<<<
...
static void ehci_hsic_reset_sof_bug_handler(struct usb_hcd *hcd, u32 val)
{
	...
	struct msm_hsic_host_platform_data *pdata = mehci->dev->platform_data;
	u32 __iomem *status_reg = &ehci->regs->port_status[0];
// P13120, [PATCH] EHCI: HSIC: Halt the controller while resetting the port [CRs-Fixed: 459280] --->>>
	u32 cmd;
// P13120, [PATCH] EHCI: HSIC: Halt the controller while resetting the port [CRs-Fixed: 459280] ---<<<
...
// P13120, [PATCH] EHCI: HSIC: Fix GPT timer count values used for resume sequence [CRs-Fixed: 449090] --->>>
...
// P13120, [PATCH] EHCI: HSIC: Fix a race between root hub resume and controller removal [CRs-Fixed: 444928] --->>>
